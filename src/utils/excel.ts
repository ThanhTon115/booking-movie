...
export function convertObjectsToRows<T = Record<string, any>>(
  input: T[],
  keys?: Array<string | ((item: T, rowIndex?: number, columnIndex?: number) => unknown)>
): unknown[][] {
  if (!input?.length) {
    return []
  }
  ...
  return input.map((m, y) => keys.map((k, x) => (typeof k === 'function' ? k(m, y, x) : m[k])))
}
...
const buildBorderStyle = (borders: ExcelBorders): Partial<Borders> => {
  ...
  borderTypes = borderTypes.filter((k) => !['top', 'left', 'bottom', 'right', 'outside'].includes(k))
...
export const createExcelBuilder = (options?: ExcelBuilderOptions) => {
  ...
  if (options) {
    Object.keys(options).forEach((k) => {
      ...
      if ((workbook as object).hasOwnProperty(k) && typeof workbook[k] !== 'function') {
        workbook[k] = k
      }
    })
  }
  ...
  const addCells = (cells: ExcelCell[], style?: ExcelStyle) => {
    cells.forEach((c) => {
      const rowPositions = [c.start.r, c.end?.r].filter((v) => typeof v === 'number').sort((a, b) => a - b)
      const colPositions = [c.start.c, c.end?.c].filter((v) => typeof v === 'number').sort((a, b) => a - b)
      ...
      if (c.end) {
        worksheet.mergeCells(rowPositions[0], colPositions[0], rowPositions[1], colPositions[1])
      }
