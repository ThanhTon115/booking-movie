...
export const compareSanitizeString = (
  ...
  return strA === strB || sanitizeString(strA).toLowerCase().includes(sanitizeString(strB).toLowerCase())
}

export const generateRandomString = (len = 16, wishlist = alphaDigits): string =>
  Array(len)
    .fill('')
    .map(
...
export const toPostgresArray = (input: string[]): string => `{${(input || []).join(',')}}`
...
export const encodeBase64 = (str: string): string => window.btoa(unescape(encodeURIComponent(str)))

export const decodeBase64 = (str: string): string => decodeURIComponent(escape(window.atob(str)))
...
export function buildKeywords(input: string | string[] | Record<string, string>): string {
  if (!input) {
    return ''
  }
  ...
  return inputs
    .map((s) => {
      ...
      return trimmed === sanitizedWords ? trimmed : `${trimmed} ${sanitizedWords}`
    })
    .filter((s) => s)
    .join(' ')
}
...
export const formatPhoneNumber = (phoneNumber: string) =>
  `${phoneNumber[0] === '0' && phoneNumber.length === 10 ? phoneNumber : `0${phoneNumber}`}`

export const toCapitalize = (s: string): string => s.replace(/\w\S*/g, (w) => w.replace(/^\w/, (c) => c.toUpperCase()))
...
export function formatCurrency(n: any, isUnit?: boolean, currency?: string): string {
  if (n === 0) {
    return `0${isUnit ? 'đ' : ''}`
  }
  if (!n) {
...
export const toAddressString = (address: any) => {
  return ['street', 'ward_name', 'district_name', 'province_name'].map((e) => address[e] || '').join(', ')
}
