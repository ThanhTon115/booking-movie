...
export const resizeImage = (file: File, options: ResizeImageOptions[]): Promise<File[]> =>
...
export type FileType = typeof FILE_TYPE_PHOTO | typeof FILE_TYPE_VIDEO | typeof FILE_TYPE_FILE
...
export function parseIconFile(file: any): string {
  const fileType: FileType = file.raw ? parseFileType(file.raw.type) : file.fileType
  ...
  switch (fileType) {
    ...
    case FILE_TYPE_FILE:
      return (() => {
        ...
        return isKnownExtension(extension) ? FILE_ICON_NAMES[extension as FileExtension] : FILE_ICON_FILE
      })()
    default:
...
export function downloadWithNoCorsFromURL(url: string, filename: string, mimeType?: string) {
  return fetch(url)
    .then((response) => response.blob())
    .then((blob) => {
      const blobURL = URL.createObjectURL(blob)
