...
const isDifferent = (
  from: unknown,
  to: unknown,
  options?: { unordered?: boolean; pick?: string[] }
): boolean => {
  ...
  return (options?.pick?.length ? options.pick : Object.keys(from)).some((k) =>
    isDifferent(from[k], to[k])
  );
};

export const difference = (from: unknown, to: unknown): boolean =>
  isDifferent(from, to);
...
export function distinctBy<T = unknown>(
  input: T[],
...
export function merge<T = unknown>(src: T, dest: T): T {
  ...
  return distinct([...Object.keys(src), ...Object.keys(dest)]).reduce(
    (acc, key) => {
      return {
        ...acc,
...
>(inputs: T[], predicate: (item: T) => K): Record<K, T[]> {
  return (inputs || []).reduce((acc, item) => {
    const key = predicate(item);
    return {
      ...acc,
      [key]: [
        ...(Object.prototype.hasOwnProperty.call(acc, key) ? acc[key] : []),
        item,
      ],
    };
  }, {} as Record<K, T[]>);
...
export function replicate<T = unknown>(
  value: (() => T) | T,
  quantity: number
): T[] {
  if (quantity <= 0) {
...
export function uniqueBy<T = unknown>(
  inputs: T[],
...
export function mergeArray<T = unknown>(
  src: T[],
  dest: T[],
  predicate: (a: T, b: T) => boolean = identity
): T[] {
  if (!src?.length) {
    return dest;
  }
...
export function minBy<T = unknown>(
  array: T[],
...
export function maxBy<T = unknown>(
  array: T[],
  compareFn: (a: T, b: T) => number
): T | null {
  if (!array?.length) {
...
export function predicateKeywords<T = Record<string, any>>(
  keywords: string,
  keywordSelector: (item: Record<string, any>) => string = (item) =>
    item.keywords
) {
  return (item: T) =>
    !keywords || keywordSelector(item).includes(keywords.trim().toLowerCase());
}
